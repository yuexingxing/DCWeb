...
public class LoginController extends BaseController {
	...
	public ModelAndView toLogin() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("SYSNAME", Tools.readTxtFile(Const.SYSNAME)); // 读取系统名称("admin/config/SYSNAME.txt")
		...
		if (null != KEYDATA && KEYDATA.length == 3) {
			...
			if (null == code || "".equals(code)) {
				...
				if (Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)) {
					...
					/**
					 * {PHONE=18788888888, LAST_LOGIN=2017-04-24 21:27:53, NAME=系统管理员, 
					 * RIGHTS=1133671055321055258374707980945218933803269864762743594642571294, USER_ID=1, 
					 * IP=0:0:0:0:0:0:0:1, EMAIL=admin@main.com, USERNAME=admin, STATUS=0, PASSWORD=28dca2a7b33b7413ad3bce1d58c26dd679c799f1, 
					 * SKIN=skin-1, BZ=最高统治者, ROLE_ID=1, NUMBER=001}
					 */
					if (pd != null) {
			...
			if (user != null) {
				...
				if (null == session.getAttribute(Const.SESSION_menuList) || ("yes".equals(changeMenu))) {
					...
					for (int i = 0; i < allmenuList.size(); i++) {
						...
						System.out.println("------menu------" + menu.getMENU_NAME());
						for(int k=0; k<menu.getSubMenu().size(); k++){

							Menu menu2 = menu.getSubMenu().get(k);
