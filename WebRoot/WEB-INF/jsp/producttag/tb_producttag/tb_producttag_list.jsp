...
<body>
	...
	<div class="container-fluid" id="main-container">
		...
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="tb_producttag/list.do" method="post" name="Form"
						id="Form">
						<table>
							...
							<tr>
								...
								<td style="vertical-align: top; width: 50px;"><button
										class="btn btn-mini btn-light" onclick="search();" title="检索">
										<i id="nav-search-icon" class="icon-search"></i>
									</button></td>

								<td></td>
								<td><input type="button" onclick="sortOrderNum()"
									style="width: 100px; margin-left: 10px;"
									class="btn btn-mini btn-primary" value="排序" />
								<td>
							</tr>
										...
										<c:if test="${QX.cha == 1 }">
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													...
													<td class="center">${var.TAG_CODE}</td>
													<td class="center">${var.ORDER_NUM}</td>
													<td class="center">${var.ACTIVED}</td>
	...
	<script type="text/javascript">
		...
		//排序
		function sortOrderNum(){
		
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="排序";
			 diag.URL = '<%=basePath%>tb_producttag/goSortOrderNum.do';
			 diag.Width = 400;
			 diag.Height = 120;
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>tb_producttag/goAdd.do';
			 diag.Width = 320;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>tb_producttag/delete.do?ID="+Id;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>tb_producttag/goEdit.do?ID='+Id;
			 diag.Width = 320;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
					top.jzts();
	...
	<script type="text/javascript">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>tb_producttag/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>tb_producttag/excel.do';
		}
		</script>
