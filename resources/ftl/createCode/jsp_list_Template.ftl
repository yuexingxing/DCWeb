...
<%@ include file="/WebRoot/WEB-INF/jsp/system/admin/top.jsp"%>
</head>
<body>
	...
	<div class="container-fluid" id="main-container">
		...
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="${objectNameLower}/list.do" method="post" name="Form"
						id="Form">
						<table>
							<tr>
								...
								<td><input class="span10 date-picker" name="lastLoginStart"
									id="lastLoginStart" value="${r"
									${pd.lastLoginStart}"}" type="text"
									data-date-format="yyyy-mm-dd" readonly="readonly"
									style="width: 88px;" placeholder="开始日期" /></td>
								<td><input class="span10 date-picker" name="lastLoginEnd"
									id="lastLoginEnd" value="${r" ${pd.lastLoginEnd}"}" type="text"
									data-date-format="yyyy-mm-dd" readonly="readonly"
						...
						<table id="table_report"
							...
							<thead>
								<tr>
									...
									<th class="center">序号</th> <#list fieldList as var>
									<th class="center">${var[2]}</th> </#list>
											...
											<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
												<tr>
													<td class='center' style="width: 30px;"><label><input
															type='checkbox' name='ids' value="${r" ${var."}${objectNameUpper}_ID${r"}"}" /><span
															class="lbl"></span></label></td>
													<td class='center' style="width: 30px;">${r"${vs.index+1}"}</td>
													<#list fieldList as var>
													<td>${r"${var."}${var[0]}${r"}"}</td> </#list>
																...
																<ul
																	...
																	<c:if test="${r"${QX.edit == 1 }"}">
																		<li><a style="cursor: pointer;" title="编辑"
																			onclick="edit('${r"${var."}${objectNameUpper}_ID${r"}"}');" class="tooltip-success"
																	...
																	<c:if test="${r"${QX.del == 1 }"}">
																		<li><a style="cursor: pointer;" title="删除"
																			onclick="del('${r"${var."}${objectNameUpper}_ID${r"}"}');" class="tooltip-error"
																			data-rel="tooltip" title="" data-placement="left"><span
						...
						<div class="page-header position-relative">
							<table style="width: 100%;">
								<tr>
									<td style="vertical-align: top;"><c:if test="${r"${QX.add == 1 }"}">
											<a class="btn btn-small btn-success" onclick="add();">新增</a>
										</c:if> <c:if test="${r"${QX.del == 1 }"}">
